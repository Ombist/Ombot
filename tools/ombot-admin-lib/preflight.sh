...
ombist_cmd_preflight_main() {
  ...
  ombist_has_node() {
    ...
    for cand in /usr/local/bin/node /usr/bin/node /usr/bin/nodejs /opt/ombot/npm-global/bin/node; do
  ...
  ombist_has_npm() {
    ...
    for cand in /usr/local/bin/npm /usr/bin/npm /opt/ombot/npm-global/bin/npm; do
      [ -x "$cand" ] && return 0
    done
    return 1
  }
